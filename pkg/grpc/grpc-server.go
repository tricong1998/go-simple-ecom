...
func NewGrpcServer(log zerolog.Logger, config *GrpcConfig) *GrpcServer {
	s := grpc.NewServer(
...
func (s *GrpcServer) RunGrpcServer(ctx context.Context, configGrpc ...func(grpcServer *grpc.Server)) error {
	...
	go func() {
		for {
			select {
			case <-ctx.Done():
				s.Log.Info().Msgf("shutting down grpc PORT: {%s}", s.Config.Port)
				s.shutdown()
				s.Log.Info().Msg("grpc exited properly")
				return
			}
		}
	}()

	s.Log.Info().Msgf("grpc server is listening on port: %s", s.Config.Port)

	err = s.Grpc.Serve(listen)

	if err != nil {
		s.Log.Error().Msgf("[grpcServer_RunGrpcServer.Serve] grpc server serve error: %+v", err)
	}
